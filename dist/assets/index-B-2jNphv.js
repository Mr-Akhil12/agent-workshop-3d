...
function createEnvMap(renderer) {
  ...
  envScene.background = new THREE.Color(526348);
  const ceilLight = new THREE.PointLight(16774630, 60, 30);
  ceilLight.position.set(0, 8, 0);
  envScene.add(ceilLight);
  const sideLight = new THREE.PointLight(16768426, 30, 25);
  sideLight.position.set(4, 5, 2);
  envScene.add(sideLight);
  const floor = new THREE.Mesh(
    new THREE.PlaneGeometry(20, 20),
    new THREE.MeshBasicMaterial({ color: 657930 })
  );
  floor.rotation.x = -Math.PI / 2;
  envScene.add(floor);
  const backWall = new THREE.Mesh(
    new THREE.PlaneGeometry(10, 6),
    new THREE.MeshBasicMaterial({ color: 789520 })
  );
  backWall.position.set(0, 3, -5);
  envScene.add(backWall);
  const h1 = new THREE.Mesh(
    new THREE.BoxGeometry(0.3, 0.02, 0.3),
    new THREE.MeshBasicMaterial({ color: 16775408 })
  );
  h1.position.set(0, 4, -4.9);
  envScene.add(h1);
  const h2 = new THREE.Mesh(
    new THREE.BoxGeometry(0.2, 0.2, 0.02),
    new THREE.MeshBasicMaterial({ color: 15658751 })
  );
...
function makeFlakeBlackPaint(envMap) {
  return new THREE.MeshPhysicalMaterial({
    ...
    sheen: 0.5,
    sheenRoughness: 0.15,
    sheenColor: new THREE.Color(6710920)
  });
}
(function() {
  ...
  scene.background = new THREE.Color(394762);
  scene.fog = new THREE.FogExp2(394762, 0.06);
  ...
  scene.add(new THREE.AmbientLight(1710626, 0.3));
  const keyLight = new THREE.DirectionalLight(16774630, 1.2);
  ...
  scene.add(keyLight);
  const fillLight = new THREE.DirectionalLight(8952251, 0.2);
  fillLight.position.set(-5, 4, -3);
  scene.add(fillLight);
  const rimLight = new THREE.DirectionalLight(16768426, 0.15);
  ...
  function createHexLight(x, y, z) {
    ...
    const frameMat = new THREE.MeshStandardMaterial({ color: 2763306, metalness: 0.8, roughness: 0.3 });
    ...
    group.add(panel);
    const light = new THREE.PointLight(16774630, 1.5, 6, 2);
  ...
  scene.add(floor);
  const wallMat = new THREE.MeshStandardMaterial({ color: 789522, metalness: 0.1, roughness: 0.9 });
  ...
  const roof = new THREE.Mesh(
    ...
    new THREE.MeshStandardMaterial({ color: 657936, metalness: 0.2, roughness: 0.8 })
  );
  roof.position.set(0, 3.55, -1.5);
  roof.castShadow = true;
  scene.add(roof);
  const beamMat = new THREE.MeshStandardMaterial({ color: 1710624, metalness: 0.4, roughness: 0.5 });
  ...
  const border = new THREE.LineSegments(
    ...
    new THREE.LineBasicMaterial({ color: 4500223 })
  );
  ...
  const poleMesh = new THREE.Mesh(
    ...
    new THREE.MeshStandardMaterial({ color: 2236968, metalness: 0.7, roughness: 0.3 })
  );
  ...
  const capMesh = new THREE.Mesh(
    ...
    new THREE.MeshStandardMaterial({ color: 8947865, metalness: 0.8, roughness: 0.2 })
  );
  ...
  signData.forEach((s, i) => {
    ...
    const body = new THREE.Mesh(
      ...
      new THREE.MeshStandardMaterial({ color: 1710624, metalness: 0.3, roughness: 0.6 })
    );
  ...
  const barrelMat = new THREE.MeshStandardMaterial({ color: 1118481, metalness: 0.4, roughness: 0.6 });
  ...
  const shelf = new THREE.Mesh(new THREE.BoxGeometry(0.4, 0.06, 1.8), new THREE.MeshStandardMaterial({ color: 1710618, metalness: 0.3, roughness: 0.6 }));
  ...
  function buildLaptop() {
    const group = new THREE.Group();
    const baseMat = new THREE.MeshStandardMaterial({ color: 1710618, metalness: 0.8, roughness: 0.2 });
    group.add(new THREE.Mesh(new THREE.BoxGeometry(0.8, 0.04, 0.55), baseMat));
    const screenM = new THREE.MeshStandardMaterial({ color: 1118481, metalness: 0.9, roughness: 0.1 });
    ...
    const kb = new THREE.Mesh(new THREE.PlaneGeometry(0.65, 0.35), new THREE.MeshStandardMaterial({ color: 657930, metalness: 0.3, roughness: 0.8 }));
  ...
  function createFlameSystem() {
    ...
    const mat = new THREE.PointsMaterial({ color: 16737792, size: 0.1, transparent: true, opacity: 0.7, blending: THREE.AdditiveBlending, depthWrite: false, sizeAttenuation: true });
    flameParticles = new THREE.Points(geo, mat);
    flameParticles.userData = { velocities, lifetimes };
    scene.add(flameParticles);
    flameLight = new THREE.PointLight(16729088, 0, 4, 2);
